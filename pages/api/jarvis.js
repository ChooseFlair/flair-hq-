...
async function getTrueLayerToken() {
  ...
  if (new Date(data.expires_at) < new Date(Date.now() + 60000)) {
    try {
      ...
      await supabase.from('personal_bank_tokens').upsert({ id: 'truelayer', access_token: tokens.access_token, refresh_token: tokens.refresh_token, expires_at: new Date(Date.now() + tokens.expires_in * 1000).toISOString(), updated_at: new Date().toISOString() })
...
async function getOrdersSummary() {
  const { data: orders } = await supabase.from('orders').select('total_price, financial_status, fulfillment_status, created_at, email').order('created_at', { ascending: false }).limit(500)
...
async function getRecentOrders(limit = 10) {
  const { data } = await supabase.from('orders').select('order_number, email, total_price, financial_status, fulfillment_status, created_at').order('created_at', { ascending: false }).limit(limit)
  return data || []
}

async function getCustomerInsights() {
  const { data } = await supabase.from('customers').select('email, total_spent, order_count').order('total_spent', { ascending: false }).limit(20)
  return { top_customers: (data || []).map(c => ({ email: c.email, spent: c.total_spent, orders: c.order_count })), total_in_db: data?.length || 0 }
}

async function getKlaviyoMetrics() {
  const headers = { 'Authorization': `Klaviyo-API-Key ${KLAVIYO_KEY}`, 'revision': '2024-10-15', 'Content-Type': 'application/json', 'Accept': 'application/json' }
  try {
    ...
    const profiles = await profilesRes.json(), flows = await flowsRes.json(), lists = await listsRes.json()
    return { total_profiles: profiles.meta?.total || 'unknown', flows: (flows.data || []).map(f => ({ name: f.attributes.name, status: f.attributes.status })), lists: (lists.data || []).map(l => ({ name: l.attributes.name, id: l.id })) }
...
async function getMetaAdsData() {
  ...
  return { latest_daily: daily?.[0] || null, campaigns: (campaigns || []).map(c => ({ name: c.campaign_name, status: c.status, spend: c.spend, roas: c.roas, purchases: c.purchases })) }
}
...
async function getPnLSummary() {
  ...
  return { total_revenue: totalRevenue.toFixed(2), total_orders: orders?.length || 0, recent_months_ad_spend: (overrides || []).map(o => ({ month: o.month, meta: o.meta_spend, google: o.google_spend })), total_ad_spend: totalAdSpend.toFixed(2), estimated_roas: totalAdSpend > 0 ? (totalRevenue / totalAdSpend).toFixed(2) : 'N/A' }
}

async function getWindsorData() {
  try {
    const to = new Date().toISOString().split('T')[0], from = new Date(Date.now() - 30 * 86400000).toISOString().split('T')[0]
    const res = await fetch(`https://connectors.windsor.ai/all?api_key=${WINDSOR_KEY}&fields=source,spend,clicks,impressions,date&date_from=${from}&date_to=${to}&_renderer=json`, { signal: AbortSignal.timeout(15000) })
    if (!res.ok) return { error: `Windsor ${res.status}` }
    const data = await res.json()
    const rows = Array.isArray(data) ? data : (data?.data || [])
    const bySource = {}
    for (const r of rows) { const src = r.source || 'unknown'; if (!bySource[src]) bySource[src] = { spend: 0, clicks: 0, impressions: 0 }; bySource[src].spend += parseFloat(r.spend || 0); bySource[src].clicks += parseInt(r.clicks || 0); bySource[src].impressions += parseInt(r.impressions || 0) }
...
async function getPersonalBankAccounts() {
  ...
  if (!token) return { error: 'Personal banking not connected. Visit /api/truelayer/auth to connect.' }
  try {
    const accounts = await getAccounts(token)
    const withBalances = await Promise.all(accounts.map(async (acc) => {
      try { const bal = await getBalance(token, acc.account_id); return { name: acc.display_name || acc.provider?.display_name || 'Account', provider: acc.provider?.display_name || 'Unknown', type: acc.account_type, balance: bal?.current || 0, available: bal?.available || 0, currency: bal?.currency || 'GBP' } }
      catch { return { name: acc.display_name, provider: acc.provider?.display_name, balance: 'unavailable' } }
    }))
...
async function getPersonalTransactions() {
  ...
  try {
    ...
    const to = new Date().toISOString().split('T')[0], from = new Date(Date.now() - 30 * 86400000).toISOString().split('T')[0]
    const allTxns = []
    for (const acc of accounts.slice(0, 4)) { try { const txns = await getTransactions(token, acc.account_id, from, to); allTxns.push(...txns.map(t => ({ account: acc.display_name || acc.provider?.display_name, date: t.timestamp?.split('T')[0], description: t.description, amount: t.amount, currency: t.currency, type: t.transaction_type, category: t.transaction_category }))) } catch {} }
    allTxns.sort((a, b) => new Date(b.date) - new Date(a.date))
    return { period: `${from} to ${to}`, transaction_count: allTxns.length, total_spent: allTxns.filter(t => t.amount < 0).reduce((s, t) => s + Math.abs(t.amount), 0).toFixed(2), total_income: allTxns.filter(t => t.amount > 0).reduce((s, t) => s + t.amount, 0).toFixed(2), recent_transactions: allTxns.slice(0, 20) }
  } catch (e) { return { error: e.message } }
}
...
async function getPersonalSpendingBreakdown() {
  ...
  try {
    ...
    const to = new Date().toISOString().split('T')[0], from = new Date(Date.now() - 30 * 86400000).toISOString().split('T')[0]
    const allTxns = []
    for (const acc of accounts.slice(0, 4)) { try { const txns = await getTransactions(token, acc.account_id, from, to); allTxns.push(...txns) } catch {} }
    const categories = {}
    for (const t of allTxns.filter(t => t.amount < 0)) { const cat = t.transaction_category || 'Other'; if (!categories[cat]) categories[cat] = { total: 0, count: 0 }; categories[cat].total += Math.abs(t.amount); categories[cat].count += 1 }
    return { period: `${from} to ${to}`, categories: Object.entries(categories).sort((a, b) => b[1].total - a[1].total).map(([name, d]) => ({ category: name, total: d.total.toFixed(2), transactions: d.count })), total_spent: Object.values(categories).reduce((s, c) => s + c.total, 0).toFixed(2) }
  } catch (e) { return { error: e.message } }
}

async function getPayPalData() {
  try {
    const [balance, txns] = await Promise.all([getPayPalBalance(), getPayPalTxns({ days: 30 })])
    return { balance: balance?.balances?.[0] || null, recent_transactions: (txns || []).slice(0, 15).map(t => ({ date: t.transaction_info?.transaction_initiation_date?.split('T')[0], amount: t.transaction_info?.transaction_amount?.value, currency: t.transaction_info?.transaction_amount?.currency_code, name: t.payer_info?.payer_name?.alternate_full_name || 'N/A' })), transaction_count: (txns || []).length }
  } catch (e) { return { error: e.message } }
}
...
async function getRevolutData() {
  try {
    ...
    return { accounts: (accounts || []).map(a => ({ name: a.name, balance: a.balance, currency: a.currency, state: a.state })), total_gbp: (accounts || []).filter(a => a.currency === 'GBP').reduce((s, a) => s + (a.balance || 0), 0) }
  } catch (e) { return { error: e.message } }
}

async function getFacebookData() {
  try {
    const [pageInfo, insights, posts] = await Promise.all([getPageInfo(), getPageInsights('day', 'last_30d'), getPagePosts(10)])
    return { page: { name: pageInfo?.name, followers: pageInfo?.followers_count, likes: pageInfo?.fan_count }, insights_30d: insights || {}, recent_posts: (posts?.data || []).slice(0, 5).map(p => ({ message: p.message?.substring(0, 80), created: p.created_time?.split('T')[0], likes: p.likes?.summary?.total_count || 0, comments: p.comments?.summary?.total_count || 0 })) }
  } catch (e) { return { error: e.message } }
}
...
async function getInstagramData() {
  try {
    ...
    return { username: igAccount.username, followers: igAccount.followers_count, media_count: igAccount.media_count, insights: insights || {} }
  } catch (e) { return { error: e.message } }
}
...
const TOOL_DEFS = TOOLS.map(t => ({ name: t.name, description: t.description, input_schema: { type: 'object', properties: {}, required: [] } }))
...
export default async function handler(req, res) {
  ...
  try {
    const claudeRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 1024, system: SYSTEM_PROMPT, tools: TOOL_DEFS, messages }),
    })
    if (!claudeRes.ok) { const err = await claudeRes.text(); return res.status(502).json({ error: `Claude API ${claudeRes.status}: ${err.substring(0, 200)}` }) }

    let result = await claudeRes.json()
    let toolResults = [], allMessages = [...messages]
    ...
    while (result.stop_reason === 'tool_use') {
      ...
      for (const call of toolCalls) {
        ...
        let toolResult
        try { toolResult = tool ? await tool.fn() : { error: 'Unknown tool' } } catch (e) { toolResult = { error: e.message } }
        batchResults.push({ type: 'tool_result', tool_use_id: call.id, content: JSON.stringify(toolResult) })
        toolResults.push(call.name)
      }
      allMessages = [...allMessages, { role: 'assistant', content: result.content }, { role: 'user', content: batchResults }]
      const followUp = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST',
        headers: { 'x-api-key': ANTHROPIC_KEY, 'anthropic-version': '2023-06-01', 'content-type': 'application/json' },
        body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 1024, system: SYSTEM_PROMPT, tools: TOOL_DEFS, messages: allMessages }),
      })
    ...
    res.json({ response: textBlocks.map(c => c.text).join('\n') || 'I could not generate a response.', tools_used: [...new Set(toolResults)] })
  } catch (e) {
