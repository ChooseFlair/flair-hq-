...
function MonthTab({ month, active, onClick }) {
  const label = month === 'total' ? 'ALL' : new Date(month + '-01').toLocaleDateString('en-GB', { month: 'short', year: '2-digit' }).toUpperCase()
  return (
    <button onClick={onClick} className={`px-3 py-1 rounded text-[10px] font-mono tracking-wider transition-all ${active ? 'bg-cyan-500/15 text-cyan-300 border border-cyan-500/30' : 'text-cyan-500/30 hover:text-cyan-400/50 border border-transparent'}`}>
      {label}
    </button>
  )
}
        ...
        <div className="flex gap-3 max-w-5xl mx-auto">
          {[1,2,3,4,5].map(i => (
            <div key={i} className="flex-1 bg-white/[0.02] border border-cyan-500/5 rounded-lg px-4 py-3 animate-pulse">
              <div className="h-2 w-12 bg-cyan-500/10 rounded mb-2" /><div className="h-5 w-16 bg-cyan-500/10 rounded" />
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="px-6 py-3 border-b border-cyan-500/10">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-5 gap-2">
          ...
          <MetricCard label="COGS" value={fmt(m.cogs)} sub={`${data.rates?.cogs ? (data.rates.cogs * 100).toFixed(1) : 20.5}% of rev`} />
...
export default function Home() {
  ...
  function handleAgentAsk(query) {
    if (jarvisRef.current?.send) {
      jarvisRef.current.send(query)
    }
  }
  ...
  return (
    <>
      ...
      <div className="h-screen flex flex-col bg-black overflow-hidden">
        ...
        <div className="flex-1 min-h-0 border-t border-cyan-500/10">
