...
export default function Finance({ activeSubTab, setActiveSubTab }) {
  ...
  const forecastData = useMemo(() => {
    ...
    const monthlyIncome = getMonthlyIncome() / 100
    const monthlySpend = getMonthlySpend() / 100
    const paypalMonthlyIncome = paypalData?.summary?.totalIncome || 0
    const paypalMonthlyFees = paypalData?.summary?.totalFees || 0

    // Total monthly revenue and costs
    const totalMonthlyRevenue = monthlyIncome + paypalMonthlyIncome
    const totalMonthlyExpenses = monthlySpend + paypalMonthlyFees

    // Current balance
    const currentBalance = getTotalBalance() / 100
    const paypalBalance = paypalData?.balance?.total_balance?.value ? parseFloat(paypalData.balance.total_balance.value) : 0
    const totalBalance = currentBalance + paypalBalance
    ...
    for (let i = 0; i <= projectionMonths; i++) {
      ...
      if (i === 0) {
        ...
        const conservativeGrowth = 1.0 // 0% growth
        const moderateGrowth = 1.05 // 5% monthly growth
        const optimisticGrowth = 1.10 // 10% monthly growth

        const projectedRevenue = totalMonthlyRevenue * Math.pow(moderateGrowth, i - 1)
        const projectedExpenses = totalMonthlyExpenses * 1.02 // 2% expense increase
    ...
    // Runway calculation
    const runway = monthlyProfit < 0 && totalBalance > 0
      ? totalBalance / Math.abs(monthlyProfit)
      : monthlyProfit >= 0 ? Infinity : 0

    // Revenue targets (for income replacement)
    const targetMonthlyIncome = 3000 // £3k/month target
    const gapToTarget = targetMonthlyIncome - totalMonthlyRevenue
    const monthsToTarget = gapToTarget > 0 && totalMonthlyRevenue > 0
      ? Math.ceil(Math.log(targetMonthlyIncome / totalMonthlyRevenue) / Math.log(1.05))
      : 0
    ...
    return {
      ...
      targets: {
        ...
        monthsToTarget: monthsToTarget > 0 ? monthsToTarget : 'Achieved!',
      },
      scenarios: {
        conservative: projections.map((p, i) => ({
          ...p,
          balance: i === 0 ? p.balance : totalBalance + (totalMonthlyRevenue - totalMonthlyExpenses) * i
        })),
        moderate: projections,
        optimistic: projections.map((p, i) => ({
          ...p,
          balance: i === 0 ? p.balance : totalBalance + ((totalMonthlyRevenue * Math.pow(1.10, i - 1)) - totalMonthlyExpenses * 1.02) * i
        }))
      }
    }
  }, [transactions, paypalData])
      ...
      {activeTab === 'forecast' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-sm text-gray-500">Total Balance</p>
              <p className="text-xl font-bold text-gray-900">£{forecastData.currentMetrics.totalBalance.toFixed(2)}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-sm text-gray-500">Monthly Revenue</p>
              <p className="text-xl font-bold text-green-600">£{forecastData.currentMetrics.monthlyRevenue.toFixed(2)}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-sm text-gray-500">Monthly Expenses</p>
              <p className="text-xl font-bold text-red-600">£{forecastData.currentMetrics.monthlyExpenses.toFixed(2)}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-sm text-gray-500">Monthly Profit</p>
              <p className={`text-xl font-bold ${forecastData.currentMetrics.monthlyProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                £{forecastData.currentMetrics.monthlyProfit.toFixed(2)}
              </p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-sm text-gray-500">Profit Margin</p>
              <p className={`text-xl font-bold ${forecastData.currentMetrics.profitMargin >= 0 ? 'text-purple-600' : 'text-red-600'}`}>
                {forecastData.currentMetrics.profitMargin.toFixed(1)}%
              </p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-sm text-gray-500">Runway</p>
              <p className="text-xl font-bold text-blue-600">{forecastData.currentMetrics.runway}</p>
            </div>
          </div>

          {/* 6 Month Projection Chart */}
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h3 className="font-semibold text-gray-900 mb-4">6 Month Cash Flow Projection</h3>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={forecastData.projections} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  <defs>
                  ...
                  <YAxis tick={{ fontSize: 12, fill: '#6B7280' }} tickFormatter={(v) => `£${v.toFixed(0)}`} />
                  <Tooltip content={<CustomTooltip />} />
                  <Area
                    type="monotone"
                    dataKey="balance"
            ...
            <p className="text-xs text-gray-500 mt-3">* Based on 5% monthly revenue growth and 2% expense increase</p>
          </div>

          {/* Revenue vs Expenses Projection */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h3 className="font-semibold text-gray-900 mb-4">Projected Monthly P&L</h3>
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={forecastData.projections.slice(1)} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                    <XAxis dataKey="month" tick={{ fontSize: 11, fill: '#6B7280' }} />
                    <YAxis tick={{ fontSize: 11, fill: '#6B7280' }} tickFormatter={(v) => `£${v.toFixed(0)}`} />
                    <Tooltip content={<CustomTooltip />} />
            ...
            <div className="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-200 rounded-xl p-6">
              <h3 className="font-semibold text-gray-900 mb-4">Income Replacement Target</h3>
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-gray-600">Progress to £3,000/month</span>
                    <span className="font-medium text-purple-600">
                      £{forecastData.currentMetrics.monthlyRevenue.toFixed(0)} / £{forecastData.targets.targetMonthlyIncome}
                    </span>
                  </div>
                  <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-purple-500 to-indigo-500 rounded-full transition-all"
                      style={{ width: `${Math.min(100, (forecastData.currentMetrics.monthlyRevenue / forecastData.targets.targetMonthlyIncome) * 100)}%` }}
                    />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div>
                    ...
                    <p className="text-xl font-bold text-purple-600">£{forecastData.targets.gapToTarget.toFixed(0)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Months to Target</p>
                    <p className="text-xl font-bold text-indigo-600">
                      {typeof forecastData.targets.monthsToTarget === 'number'
                        ? `~${forecastData.targets.monthsToTarget}`
                        : forecastData.targets.monthsToTarget}
                    </p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {forecastData.projections.map((row, idx) => (
                    <tr key={row.month} className={`hover:bg-gray-50 ${row.isProjection ? 'bg-blue-50/30' : ''}`}>
                      <td className="px-6 py-3 text-sm font-medium text-gray-900">
                        {row.month}
                        {row.isProjection && <span className="ml-2 text-xs text-blue-500">(projected)</span>}
                      </td>
                      ...
                      <td className="px-6 py-3 text-sm text-red-600 font-medium">
                        {row.expenses > 0 ? `£${row.expenses.toFixed(2)}` : '—'}
                      </td>
                      <td className={`px-6 py-3 text-sm font-medium ${row.net >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {row.net !== 0 ? `£${row.net.toFixed(2)}` : '—'}
                      </td>
                      <td className="px-6 py-3 text-sm font-bold text-gray-900">£{row.balance.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
            <p className="text-sm text-amber-800">
              <strong>Note:</strong> These projections are estimates based on current trends and assume consistent growth.
              Actual results may vary based on market conditions, ad performance, and seasonal factors.
            </p>
          </div>
        </div>
      )}
