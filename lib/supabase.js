...
// Get Klaviyo sync status
export async function getKlaviyoSyncStatus() {
  const { data, error } = await supabase
    .from('integrations')
    .select('*')
    .eq('id', 'klaviyo_sync')
  ...
  return data?.access_token ? JSON.parse(data.access_token) : null
}

// Get orders for Klaviyo sync (optionally since a given timestamp)
export async function getOrdersForSync(since = null) {
  let query = supabase
    .from('orders')
    .select('*')
    .order('created_at', { ascending: false })
